...
def _interview_mode_guidance(mode: str) -> str:
    ...
    if normalized in {"practice", "practical", "coding", "code"}:
        return (
            "Режим practice (LIVE CODING) — СТРОГО:\n"
            "1) КАЖДЫЙ вопрос должен быть конкретной coding-задачей. Формулировки только из этого списка:\n"
            "   • «Реализуйте функцию X(...) на <язык>, которая делает Y»\n"
            "   • «Напишите SQL-запрос, который возвращает Z»\n"
            "   • «Напишите bash-скрипт / curl-команду / regex для ...»\n"
            "   • «Реализуйте middleware / handler / класс ...»\n"
            "   • «Допишите код по сигнатуре: func Foo(...) ...»\n"
            "2) ЗАПРЕЩЕНЫ открытые theory-вопросы: «Какой подход вы бы использовали?», "
            "«Расскажите как», «Опишите архитектуру», «Какие trade-offs» — это всё ТОЛЬКО для theory режима.\n"
            "3) Если кандидат прислал код — проверь его как код: укажи баги, edge-cases, сложность. "
            "Максимум 1-2 ФОЛЛОУ-АП по этому конкретному коду (например «что будет на пустом массиве?», "
            "«как изменится сложность для отсортированного входа?»), после чего ОБЯЗАТЕЛЬНО давай НОВУЮ задачу.\n"
            "4) Никаких бесконечных разговоров по одной задаче. Закончил обсуждение — дай следующую задачу."
        )
    if normalized in {"theory", "theoretical", "concept", "conceptual"}:
        return (
            "Режим theory — СТРОГО:\n"
            "1) Только концептуальные вопросы: trade-offs, архитектура, принципы, проверка понимания.\n"
            "2) ЗАПРЕЩЕНЫ coding-задачи («Напишите функцию», «Реализуйте...») — это для practice режима.\n"
            "3) Максимум 1-2 уточняющих вопроса по одной теме. Если кандидат ответил — переключайся на "
            "СЛЕДУЮЩУЮ тему из списка focus_areas, а не разгоняй один топик.\n"
            "4) После 2 follow-up на одну тему — обязательная смена темы. Не зацикливайся."
        )
    return "Режим practice (LIVE CODING): только конкретные coding-задачи, никаких открытых theory-вопросов."
